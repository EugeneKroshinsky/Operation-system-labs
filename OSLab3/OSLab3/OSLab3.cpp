...
int main() {
	...
	for (int i = 0; i < number_of_threads; ++i) {
		...
		threads[i] = CreateThread(NULL, 0, thread_func, (LPVOID)(&information[i]), NULL, NULL);    //Запустить заданное количество экземпляров потока marker. В качестве параметра
																									//каждому экземпляру потока marker передать его порядковый номер в запуске.
		terminated_threads[i] = false;
	}

	SetEvent(start_work);  //сигнал на начало работы всех потоков marker.
	...
	while (ended_threads != number_of_threads) {
		...
		bool is_thread_terminated = false; //проверяет остановился ли поток на этой итерации
